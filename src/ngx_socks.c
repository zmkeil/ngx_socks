...
static ngx_int_t ngx_socks_add_ports(ngx_conf_t *cf, ngx_array_t *ports,
    ngx_socks_listen_t *listen);
static char *ngx_socks_optimize_servers(ngx_conf_t *cf, ngx_array_t *ports);

static ngx_int_t ngx_socks_add_addrs(ngx_conf_t *cf, ngx_socks_port_t *mport,
    ngx_socks_conf_addr_t *addr);
#if (NGX_HAVE_INET6)
static ngx_int_t ngx_socks_add_addrs6(ngx_conf_t *cf, ngx_socks_port_t *mport,
    ngx_socks_conf_addr_t *addr);
#endif
...
ngx_uint_t  ngx_socks_max_module;
...
ngx_module_t  ngx_socks_module = {
    NGX_MODULE_V1,
    &ngx_socks_module_ctx,                  /* module context */
    ngx_socks_commands,                     /* module directives */
...
ngx_socks_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (cmd->name.data[0] == 'i') {
        ngx_conf_log_error(NGX_LOG_WARN, cf, 0,
                           ...
                           "use the \"socks\" directive instead");
    }
    ...
    *(ngx_socks_conf_ctx_t **) conf = ctx;
    ...
    ctx->main_conf = ngx_pcalloc(cf->pool,
                                 sizeof(void *) * ngx_socks_max_module);
    ...
    /*
     * the socks null srv_conf context, it is used to merge
    ...
    *cf = pcf;
    ...
    if (ngx_array_init(&ports, cf->temp_pool, 4, sizeof(ngx_socks_conf_port_t))
        != NGX_OK)
    {
        return NGX_CONF_ERROR;
    }
...
static ngx_int_t
ngx_socks_add_ports(ngx_conf_t *cf, ngx_array_t *ports,
    ngx_socks_listen_t *listen)
{
    in_port_t              p;
    ngx_uint_t             i;
...
static ngx_int_t
ngx_socks_add_addrs(ngx_conf_t *cf, ngx_socks_port_t *mport,
    ngx_socks_conf_addr_t *addr)
{
    u_char              *p;
    size_t               len;
    ...
    mport->addrs = ngx_pcalloc(cf->pool,
                               mport->naddrs * sizeof(ngx_socks_in_addr_t));
    if (mport->addrs == NULL) {
        return NGX_ERROR;
    }
...
static ngx_int_t
ngx_socks_add_addrs6(ngx_conf_t *cf, ngx_socks_port_t *mport,
    ngx_socks_conf_addr_t *addr)
{
    u_char               *p;
    size_t                len;
    ...
    mport->addrs = ngx_pcalloc(cf->pool,
                               mport->naddrs * sizeof(ngx_socks_in6_addr_t));
    if (mport->addrs == NULL) {
        return NGX_ERROR;
    }
...
static ngx_int_t
ngx_socks_cmp_conf_addrs(const void *one, const void *two)
{
